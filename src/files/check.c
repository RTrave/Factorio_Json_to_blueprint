	...
	for (size_t i = 0; i < files->len; i++)
	{
		...
		if (access(file->name, F_OK) != 0)
		{
			...
			fprintf(stderr, "%s: %s: %s: \"%s\"\n",
				EXECUTABLE_NAME, ERROR_FILE, ERROR_FILE_NONEXISTANT, file->name);
			error = 1;
			continue;
		}
		...
		if (stat(file->name, &file_stat) == -1)
		{
			fprintf(stderr, "%s: %s: %s: %s: %s: \"%s\"\n",
				EXECUTABLE_NAME, ERROR_FUNCTION, LIB_LIBC, FUNC_STAT,
				ERROR_FILE_STATUS,
				file->name);
			return (1);
		}
		...
		{
			fprintf(stderr, "%s: %s: %s: \"%s\"\n",
				EXECUTABLE_NAME, ERROR_FILE, ERROR_FILE_NONREGULAR, file->name);
			error = 1;
			continue;
		}
		...
		{
			fprintf(stderr, "%s: %s: %s: \"%s\"\n",
				EXECUTABLE_NAME, ERROR_FILE, ERROR_READ_PERM, file->name);
			error = 1;
		}
		if (GET_PERMISSION(file->info, WRITE) != 0 && access(file->name, W_OK) != 0)
		{
			fprintf(stderr, "%s: %s: %s: \"%s\"\n",
				EXECUTABLE_NAME, ERROR_FILE, ERROR_WRITE_PERM, file->name);
			error = 1;
		}
	}
	return (error);
